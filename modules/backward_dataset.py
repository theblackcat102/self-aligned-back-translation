...
def load_oasst_export(
    ...
    def process_thread(thread):
        if mode == "sft":
            conversations = [m.text for m in thread]
            for idx in range(0, len(conversations), 2):
                prompter = conversations[idx]
                reply = conversations[idx+1]
                conversations[idx+1] = prompter
                conversations[idx] = reply
...
if __name__ == "__main__":
    train, val = load_oasst_export('2023-04-12_oasst_ready.trees.jsonl.gz')
    for answer in val[0]:
        print(answer[:100])
